...
PNG_CANDIDATES = [
    ...
    PROJECT_ROOT / "offline/products/snapshots/ipt_fase2_semantic/esri_overlay_grid.png",
]
...
def build_grids(df):
    ...
    z_building_m = np.full((GRID_ROWS, GRID_COLS), np.nan)
    ...
    for _, r in df.iterrows():
        ...
        building = float(r.get("z_building_m", 0.0))
        ...
        if building > 0:
            z_building_m[row, col] = building

        if total > 0:
            z_total_m[row, col] = total

    # IPT North-Up: sem fix_like_v42.
    z_pino_cm = (z_total_m / TETO_MAX_M) * PINO_MAX_CM
    z_pino_cm = np.clip(z_pino_cm, 0, PINO_MAX_CM)

    return z_terrain_m, z_building_m, z_total_m, z_pino_cm
...
def add_north_scale(fig):
    # Norte textual limpo, sempre para cima.
    # Substitui a imagem antiga north_arrow_scale.png.
    fig.add_annotation(
        x=0.035,
        y=0.095,
        xref="paper",
...
def add_grid_lines(fig):
    for c in range(GRID_COLS + 1):
        fig.add_trace(go.Scatter(
            ...
            y=[-0.5, GRID_ROWS - 0.5],
            mode="lines",
            line=dict(color="rgba(190,190,190,0.85)", width=1),
            showlegend=False,
            hoverinfo="skip",
            name="GRID",
        ))
...
def add_annotations(fig, cell_mode):
    # USP lateral esquerdo removido visualmente.
...
def make_fig(layers, step, cell_mode, projector_mode):
    ...
    if "building" in layers:
        fig.add_trace(go.Heatmap(
            ...
            colorscale="Cividis",
            showscale=True,
            colorbar=dict(title="Edifícios (m)"),
            opacity=0.55,
            name="Z edifício"
        ))
    ...
    if "contour_building" in layers:
        add_contours(fig, BUILDING_CONTOURS, "EDIF", "yellow", "dot", "cota_building" in layers)
...
Z_TERRAIN_M, Z_BUILDING_M, Z_TOTAL_M, Z_PINO_CM = build_grids(DF)
...
BUILDING_CONTOURS = contour_segments(Z_BUILDING_M, interval_m=5.0)
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div(id="menu-body", children=[
            ...
            html.Div("Domínio de análise", style={"fontWeight": "bold"}),
            dcc.RadioItems(
                id="analysis-domain",
                options=[
                    {"label": "Terreno", "value": "terrain"},
                    {"label": "Edifícios", "value": "building"},
                    {"label": "Z total = terreno + edifício", "value": "total"},
                ],
                value="total",
            ),
            ...
            html.Div("Visualização", style={"fontWeight": "bold"}),
            dcc.Checklist(
                id="view-options",
                options=[
                    {"label": "Cor", "value": "color"},
                    {"label": "Curvas de nível", "value": "contour"},
                    {"label": "Cotas", "value": "cota"},
                ],
                value=["color", "contour", "cota"],
            ),

            html.Hr(),

            html.Div("Estrutura", style={"fontWeight": "bold"}),
            dcc.Checklist(
            ...
            html.Div("Mesa", style={"fontWeight": "bold"}),
            dcc.Checklist(
                id="table-options",
                options=[
...
@app.callback(
    Output("cityspace-v128719", "figure"),
    Output("status", "children"),
    Output("graph-wrap", "style"),
    Input("analysis-domain", "value"),
    Input("view-options", "value"),
    Input("structure-options", "value"),
    Input("table-options", "value"),
    Input("step-store", "data"),
    Input("cell-mode", "value"),
    Input("mesa-real", "value"),
)
def update_ui(analysis_domain, view_options, structure_options, table_options, step, cell_mode, mesa_real):
    ...
    structure_options = structure_options or []
    table_options = table_options or []

    layers = []

    mesa_pinos_on = "pinos" in table_options

    # Mesa/Pinos representa sempre o Z total levado à mesa.
    # Portanto, quando ligada, substitui a camada de cor para evitar duas legendas.
    if mesa_pinos_on:
        layers.append("pinos")
        effective_domain = "total"
    else:
        effective_domain = analysis_domain
        if "color" in view_options:
            layers.append(effective_domain)

    if "contour" in view_options:
        if effective_domain == "terrain":
            layers.append("contour_terrain")
        elif effective_domain == "building":
            layers.append("contour_building")
        else:
            layers.append("contour_total")

    if "cota" in view_options:
        if effective_domain == "terrain":
            layers.append("cota_terrain")
        elif effective_domain == "building":
            layers.append("cota_building")
        else:
            layers.append("cota_total")

    if "grid" in structure_options:
        layers.append("grid")

    fig = make_fig(layers, step, cell_mode, projector_mode)
...
@app.callback(
    Output("globe-context-box", "children"),
    Input("url", "search"),
)
def update_globe_context(search):
    if not search:
        return "IPT-CitySpace aberto diretamente, sem parâmetros do Globe-CitySpace."
...
if __name__ == "__main__":
    print("========================================")
    print("V128.7.28 — IPT SEMANTIC MESA TOTAL")
    print("========================================")
    print("CSV:", CSV_PATH)
    print("PNG:", PNG_PATH)
