...
# =========================================
# V123 — MASK FROM IPT RASTER
# =========================================
def build_mask_from_raster(png_path, rows, cols, threshold=0.15):
...
def make_fig(layers):
    ...
    if "mask" in layers:
        fig.add_trace(go.Heatmap(
            z=np.where(valid_mask, 1, np.nan),
            colorscale=[[0, "rgba(255,255,255,0)"], [1, "rgba(255,255,255,0.22)"]],
            showscale=False,
            opacity=0.35,
            hoverinfo="skip",
            name="VALID_MASK",
...
# =========================================
# V122 MASK — DOMÍNIO FÍSICO/URBANÍSTICO DA MESA
# Regras:
# 1) remove ilhas pequenas
# 2) remove zeros / NaN
# 3) aplica a mesma máscara em pinos, teto, terreno e contours
# =========================================

height_mask = np.isfinite(z_total_cm) & (z_total_cm > 0)

# =========================================
# V123 — MÁSCARA DERIVADA DO RASTER IPT
# usa o mesmo PNG base carregado em PNG_PATH
# =========================================
raster_mask = build_mask_from_raster(PNG_PATH, GRID_ROWS, GRID_COLS, threshold=0.15)

# aplicar mesma orientação do grid visual
raster_mask = fix_like_v42(raster_mask).astype(bool)

valid_mask = valid_mask & height_mask & raster_mask
    ...
    html.Div([
        ...
        dcc.Checklist(
            ...
            options=[
                ...
                {"label": "Máscara válida", "value": "mask"},
            ],
            value=["base", "pinos", "contour_total", "cota_total"],
        ),
    ], style={
        "position": "fixed",
        "left": "8px",
...
if __name__ == "__main__":
    print("========================================")
    print("V123 — IPT RASTER MASK + LAYERS + COTAS")
    print("========================================")
    print("CSV:", CSV_PATH)
    print("PNG:", PNG_PATH)
    ...
    print("Valid cells IPT raster mask:", int(valid_mask.sum()))
    print("========================================")
    app.run(debug=True)
