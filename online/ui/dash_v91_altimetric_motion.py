...
for i in range(1, len(p)):
    ...
    altura = 0.12 * abs(v[i])  # peso da altura (ajustável)
    tl.append(tl[-1] + dist + altura)
...
def rd(t):
    ...
    if s < len(p):
        r, c = p[s]
        blink = 0.5 + 0.5 * np.sin(t * 6)  # efeito pulsante
        fig.add_trace(
            go.Scatter(
                x=[c + 0.5],
                y=[r + 0.5],
...
if __name__ == "__main__":
    print(">>> V91 ALTIMETRIC MOTION <<<", flush=True)
