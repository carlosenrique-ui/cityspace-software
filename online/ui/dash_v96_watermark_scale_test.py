...
WATERMARK_SCALE = 0.94
WATERMARK_X = (1.0 - WATERMARK_SCALE) / 2.0
WATERMARK_Y = 1.0 - (1.0 - WATERMARK_SCALE) / 2.0
    ...
    fig.update_layout(
        images=[
            dict(
                ...
                x=WATERMARK_X,
                y=WATERMARK_Y,
                sizex=WATERMARK_SCALE,
                sizey=WATERMARK_SCALE,
                sizing="stretch",
                opacity=0.42,
                layer="above",
            )
...
if __name__ == "__main__":
    print(">>> RUNNING WATERMARK SCALE TEST <<<", flush=True)
    print(f">>> WATERMARK_SCALE = {WATERMARK_SCALE}", flush=True)
    print(f">>> WATERMARK_X = {WATERMARK_X}", flush=True)
    print(f">>> WATERMARK_Y = {WATERMARK_Y}", flush=True)
