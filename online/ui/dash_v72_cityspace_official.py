...
def r(step):
 ...
 for i,(rr,cc) in enumerate(path):
  if i<=step and i<len(values):
   z[rr][cc]=values[i]
 fig=go.Figure(data=go.Heatmap(z=z,colorscale="Viridis",zmin=0,zmax=10))
 fig.update_layout(xaxis=dict(scaleanchor="y"),yaxis=dict(autorange="reversed"),margin=dict(l=10,r=10,t=30,b=10))
 return fig
