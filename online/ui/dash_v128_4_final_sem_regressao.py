...
def build_grids(df):
    ...
    # Pino físico normalizado: teto 0–40 m => pino 0–10 cm
...
def add_annotations(fig, cell_mode):
    ...
    fig.add_annotation(
        x=14.75,
        y=0.45,
        text=f"Escala visual: {cell_mode}",
        showarrow=False,
        align="center",
...
PNG_ARRAY = load_png_array(PNG_PATH)
PNG_IMAGE = Image.open(PNG_PATH).convert("RGBA")

NORTH_IMAGE = None
if NORTH_PATH is not None:
    NORTH_IMAGE = Image.open(NORTH_PATH).convert("RGBA")
...
if __name__ == "__main__":
    print("========================================")
    print("V128.4 — UI OPERACIONAL SEM REGRESSÃO")
    print("========================================")
    print("CSV:", CSV_PATH)
    print("PNG:", PNG_PATH)
