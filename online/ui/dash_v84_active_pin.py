...
def render(time,zoom):
 ...
 fig=go.Figure()
 fig.add_trace(go.Heatmap(z=z,colorscale="Jet",zmin=0,zmax=10,xgap=2,ygap=2,opacity=0.65,colorbar=dict(title="Altura (cm / m)")))
 if step<len(path):
  r,c=path[step]
  fig.add_trace(go.Scatter(x=[c],y=[r],mode="markers",marker=dict(color="rgba(255,255,255,0.3)",size=26),showlegend=False))
  fig.add_trace(go.Scatter(x=[c],y=[r],mode="markers",marker=dict(color="white",size=10),showlegend=False))
 fig.update_layout(
 width=base_w*scale,
 height=base_h*scale,
