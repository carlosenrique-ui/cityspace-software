...
# ============================================================
# V96 - AJUSTE SOMENTE DO WATERMARK
# Pinos / cores / heatmap preservados
# ============================================================
SCALE_X = 0.66
SCALE_Y = 0.58
OFFSET_X = -0.050
...
# clockwise: ponta direita desce, ponta esquerda funciona como pivô
ROTATE_DEG = -7.81
...
def build_watermark_base64():
    img = Image.open(WATERMARK).convert("RGBA")
    img = img.transpose(Image.Transpose.FLIP_TOP_BOTTOM)

    w, h = img.size

    # padding grande para NÃO cortar o urbanismo ao rotacionar
    pad_x = int(w * 0.80)
    pad_y = int(h * 0.80)
    ...
    # pivô próximo à ponta esquerda do urbanismo
    pivot_x = pad_x + int(w * 0.08)
    pivot_y = pad_y + int(h * 0.50)
    ...
    # crop somente no conteúdo alfa, com margem para não cortar bordas
...
if __name__ == "__main__":
    print(">>> DASH V96 WATERMARK LEFT PIVOT <<<", flush=True)
