...
def zigzag(rows, cols):
    ...
    for x in range(cols):
        ...
        for y in ys:
            traj.append((y, x))  # (row, col)
    return traj


rows, cols = grid.shape
path = zigzag(rows, cols)

# ===============================
# VALORES REAIS
# ===============================
values = [e["value_cm"] for e in EVENTS if e["type"] == "set_height_cm"]

# 🔥 CORREÇÃO: limitar tamanho
n = min(len(path), len(values))
...
TMAX = timeline[-1] if len(timeline) > 0 else 0
...
def render(time, zoom):
    ...
    val = values[step] if step < n else 0
    m = val / 100
