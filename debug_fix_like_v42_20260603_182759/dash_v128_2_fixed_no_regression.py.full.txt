...
# =========================================
# ZIGZAG — COLUNA / CANTO SUPERIOR DIREITO
# =========================================

def build_zigzag_path():
    path = []
    for c in range(GRID_COLS - 1, -1, -1):
        col_index = GRID_COLS - 1 - c
        if col_index % 2 == 0:
...
def add_annotations(fig, cell_mode):
    ...
    fig.add_annotation(
        x=14.75,
        y=0.20,
        text=f"N ↑<br>Escala visual: {cell_mode}",
        showarrow=False,
        align="center",
...
def make_fig(layers, step, cell_mode, projector_mode):
    ...
    z_display = np.where(np.isfinite(Z_TOTAL_CM) & (Z_TOTAL_CM > 0), Z_TOTAL_CM, 0.0)
    ...
    if "pinos" in layers:
        fig.add_trace(go.Heatmap(
            ...
            showscale=True,
            colorbar=dict(title="Pino<br>(cm)", x=1.02),
        ...
        fig.add_trace(go.Heatmap(
            ...
            zmin=0,
            zmax=40.0,
            colorbar=dict(title="Teto<br>(m)", x=1.12),
            hoverinfo="skip",
            name="LEGENDA_TETO_M",
        ))
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div("V128 — CitySpace Mesa Final", style={"fontWeight": "bold", "marginBottom": "8px"}),
...
if __name__ == "__main__":
    print("========================================")
    print("V128.2 — UI FINAL SEM REGRESSÃO")
    print("========================================")
    print("CSV:", CSV_PATH)
    print("PNG:", PNG_PATH)
