...
const locations = {
  ...
  ipt: { name: "IPT - Campus Principal", lat: -23.5567944, lon: -46.7373288, height: 3000 },
...
function drawGrid(west, south, east, north) {
  ...
  for (let r=0; r<TABLE_ROWS; r++) {
    for (let c=0; c<TABLE_COLS; c++) {
      ...
      const cellId =
        "P" + String(cellNumber).padStart(3, "0");
...
function setPickedReferencePoint(lat, lon) {
  ...
  document.getElementById("areaInfo").innerHTML = `
    Ponto de referência definido pelo usuário.<br>
    Lat: ${lat.toFixed(7)}<br>
    Lon: ${lon.toFixed(7)}<br>
    Escolha uma escala e clique em "Criar Área pela Escala".
  `;
}
...
clickHandler.setInputAction(
  function(click) {
    ...
    setPickedReferencePoint(lat, lon);
  },
  Cesium.ScreenSpaceEventType.LEFT_CLICK
);

document
.getElementById("enablePickPoint")
.addEventListener(
  "click",
  () => {
    ...
    document.getElementById("areaInfo").innerHTML =
      pickPointMode
      ? "Modo de seleção ativo: clique no mapa para definir o ponto de referência."
...
function exportGridJSON() {
  ...
  if (!currentGrid) {
    ...
    alert(
      "Crie uma área antes de exportar."
    );
  ...
  const dLon =
    (currentGrid.east - currentGrid.west)
    / TABLE_COLS;

  const dLat =
    (currentGrid.north - currentGrid.south)
    / TABLE_ROWS;
  ...
  for (let r=0; r<TABLE_ROWS; r++) {
    ...
    for (let c=0; c<TABLE_COLS; c++) {
      ...
      const cellId =
        "P" +
        String(cellNumber).padStart(3,"0");
